...
import { addInvoiceAction } from "../Redux/Actions/InvoiceActions";

const InvoiceModal = (props) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const toggle = useSelector((state) => state.ModalToggle);
  ...
  const GenerateInvoice = (e) => {
    ...
    dispatch(addInvoiceAction(props.info));
  };
  ...
  return (
    <div>
      <Modal show={toggle} onHide={hideModal} size="lg" centered>
        ...
        {props.ModalType === "viewOnly" ? (
          <div style={{ margin: "1rem", textAlign: "center" }}>
            <Button onClick={() => hideModal()} style={{ width: "50%" }}>
              BACK
            </Button>
          </div>
        ) : (
